...
public:
	...
	{
		if(publisher.getNumSubscribers()==0) ROS_WARN("[Sequence] Publisher(%s %s) is not being subscribed by anything. \nIf it is the first block of the sequence, you might want to give some delay before it so that other nodes can subscribe.", typeid(T).name(), topic.c_str());
...
{
public:
    NavGoal(double posX, double posY, double posZ, double oriX, double oriY, double oriZ, double oriW) : Publish("move_base_simple/goal", 1)
    {
...
public:
    ...
    {
        setUserCallback([&](const actionlib_msgs::GoalStatusArray &msg){return(!msg.status_list.empty() && msg.status_list.back().status == 3);});
    }
};
...
public:
    ...
    {
        setUserCallback([&](const actionlib_msgs::GoalStatusArray &msg){return(!msg.status_list.empty() && msg.status_list.back().status == 1);});
    }
};
